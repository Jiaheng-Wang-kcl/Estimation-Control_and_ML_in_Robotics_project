...
def getCostMatrices(num_joints):
    ...
    # Q_diag_pos = np.array([23682.75086568, 24997.46331118, 309.63705477, 4046.34774428,
    #                    21681.43017605, 24955.69311161, 24946.49254035]) # best for q fit only
    # Q_diag_vel = np.array([818.76196388, 13.12663621, 862.35380667, 1418.11312573,
    #                    628.08551276, 3.64058363, 862.62257381]) # best for q fit only

    Q_diag_pos = np.array([11424.4654835, 48213.23503945, 49999.71078435, 50000.0,
                       6609.96455797, 43984.41487614, 50000.0]) # perfect
    Q_diag_vel = np.array([812.96923028, 32.46451927, 1091.23307903, 36.48563014,
                       1876.72336268, 30.15977239, 1187.8611576]) # perfect
    Q = np.zeros((2 * num_joints, 2 * num_joints))
    ...
    # R_diag = np.array([4.11204403, 0.00922569, 2.10816267, 8.23706454, 
    #                6.07013935, 0.00488917, 12.73596369]) # best for q fit only
    R_diag = np.array([19.8754679, 0.0123199, 5.14565333, 0.02087634, 
                   5.37383007, 0.00977358, 0.73970695])
    R = np.diag(R_diag)
